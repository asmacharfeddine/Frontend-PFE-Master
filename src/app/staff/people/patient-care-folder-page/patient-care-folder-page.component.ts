...
import { ValidationFormComponent } from 'app/staff/prescription/medication-parts/validation-form/validation-form.component';
...
//this.goToPatientData();
 ...
 // using this function because the average load will load the same prescription as many times as it the medicationsParts it has
    ...
    (data: Prescription[]) => {
      ...
      for (const prescription of data) {
        ...
        const existingPrescription = this.uniquePrescriptions.find(
          (p) =>
            p.prescriptionTime === prescription.prescriptionTime &&
 ...
 showPrescriptionMedicationParts(data:any){
  this.router.navigate(['/staff/prescriptionMedicationParts',data]);
...
openValidationForm(data: any){
  ...
  dialogRef.afterClosed().subscribe((result) => {
    if (result === 1){
      this.openSuccessSnackBar("Daytake Added Succefully!");
    }
    // Handle any logic after the dialog is closed if needed
  });
  /*dialogRef.afterClosed().subscribe(result => {
...
/*countDayTakesWithCurrentDateInMedicationPart(medPart: MedicationPart): number {
  const currentDate = new Date(); // Get the current date
  const currentDateStr = currentDate; // Convert to a string for comparison

  const dayTakes = medPart.dayTakes;

  // Count the dayTakes with the current date
  const count = dayTakes.reduce((total, dayTake: DayTakes) => {
    const dayTakeDate = new Date(dayTake.takeDate);

    if (dayTakeDate === currentDateStr) {
      return total + 1;
    }
